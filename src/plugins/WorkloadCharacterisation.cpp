...
WorkloadCharacterisation::WorkloadCharacterisation(const Context* context)
    : WorkloadCharacterisation::Plugin(context)
{
...
WorkloadCharacterisation::~WorkloadCharacterisation()
{
  ...
  // present memory transfer statistics -- only run once, since these are
  // collected outside kernel invocations
  cout << "+-------------------------------------------------------------------"
          "------------------------------------+"
       << endl;
  cout << "|Memory Transfers -- statistics around host to device and device to "
          "host memory transfers               |"
       << endl;
  cout << "+==================================================================="
          "====================================+"
       << endl;
  // I can't imagine a scenario where data are copied from the device before a
  // kernel is executed. So I use the deviceToHostCopy kernel names for the
  // final statistics -- these names for the m_hostToDeviceCopy's are updated
  ...
  std::vector<std::string> unique_kernels_involved_with_device_to_host_copies =
    x;

  x = m_hostToDeviceCopy;
  unique_x = std::unique(x.begin(), x.end());
  x.resize(std::distance(x.begin(), unique_x));
  std::vector<std::string> unique_kernels_involved_with_host_to_device_copies =
    x;
  ...
  {
    cout << "\t" << item << ": "
         << std::count(m_hostToDeviceCopy.begin(), m_hostToDeviceCopy.end(),
                       item)
         << endl;
  }
  ...
  {
    cout << "\t" << item << ": "
         << std::count(m_deviceToHostCopy.begin(), m_deviceToHostCopy.end(),
                       item)
         << endl;
  }
  ...
  std::string logfile_name =
    "aiwc_memory_transfers_" + std::to_string(logfile_count) + ".csv";
  ...
  for (auto const& item : unique_kernels_involved_with_host_to_device_copies)
  {
    logfile << "transfer: host to device," << item << ","
            << std::count(m_hostToDeviceCopy.begin(), m_hostToDeviceCopy.end(),
                          item)
            << "\n";
  }
  for (auto const& item : unique_kernels_involved_with_device_to_host_copies)
  {
    logfile << "transfer: device to host," << item << ","
            << std::count(m_deviceToHostCopy.begin(), m_deviceToHostCopy.end(),
                          item)
            << "\n";
  }
...
void WorkloadCharacterisation::hostMemoryLoad(const Memory* memory,
...
void WorkloadCharacterisation::hostMemoryStore(const Memory* memory,
                                               size_t address, size_t size,
...
void WorkloadCharacterisation::threadMemoryLedger(size_t address,
                                                  uint32_t timestep,
                                                  Size3 localID)
{
...
void WorkloadCharacterisation::memoryLoad(const Memory* memory,
...
void WorkloadCharacterisation::memoryStore(const Memory* memory,
                                           const WorkItem* workItem,
                                           size_t address, size_t size,
                                           const uint8_t* storeData)
{
...
void WorkloadCharacterisation::memoryAtomicLoad(const Memory* memory,
                                                const WorkItem* workItem,
                                                AtomicOp op, size_t address,
                                                size_t size)
{
...
void WorkloadCharacterisation::memoryAtomicStore(const Memory* memory,
                                                 const WorkItem* workItem,
                                                 AtomicOp op, size_t address,
                                                 size_t size)
{
  ...
  const TypedValue& result)
{
  ...
  // get all unique labels -- for register use -- and the # of instructions
  // between loads and stores -- as the freedom to reorder
  ...
  if (auto inst = llvm::dyn_cast<llvm::LoadInst>(instruction))
  {
    ...
    m_state.instructionsBetweenLoadOrStore->push_back(
      m_state.ops_between_load_or_store);
  ...
  // collect conditional branches and the associated trace to count which ones
  // were taken and which weren't
  if (m_state.previous_instruction_is_branch == true)
  {
    ...
    else
    {
      ...
      cout << "Str was: " << Str
           << " but target was either: " << m_state.target1
           << " or: " << m_state.target2 << endl;
      std::raise(SIGINT);
    }
    m_state.previous_instruction_is_branch = false;
  }
  // if a conditional branch -- they have labels and thus temporarily store them
  // and see where we jump to in the next instruction
...
vector<double> parallelSpatialLocality(
  vector<vector<WorkloadCharacterisation::ledgerElement>> hist);

void WorkloadCharacterisation::workGroupBarrier(const WorkGroup* workGroup,
                                                uint32_t flags)
{
  ...
  {
    maxLength = m_state.ledger[i].size() > maxLength ? m_state.ledger[i].size()
                                                     : maxLength;
...
void WorkloadCharacterisation::workItemBegin(const WorkItem* workItem)
{
  ...
  // m_state.work_item_no = localID.x * m_local_num.y * m_local_num.z +
  // localID.y * m_local_num.z
  //       + localID.z;
...
{
  m_state.instructionsBetweenBarriers->push_back(m_state.instruction_count);
  m_state.instructionsPerWorkitem->push_back(
    m_state.workitem_instruction_count);
}

void WorkloadCharacterisation::kernelBegin(
  const KernelInvocation* kernelInvocation)
{
  // update the list of memory copies from host to device; since the only reason
  // to write to the device is before an execution.
...
{
  std::vector<std::unordered_map<size_t, uint32_t>> local_address_count(
    11, unordered_map<size_t, uint32_t>());
  ...
  for (int nskip = 0; nskip < 11; nskip++)
  {
    ...
    {
      double prob =
        (double)(it.second) * 1.0 / (double)(total_access_count + 1);
...
void WorkloadCharacterisation::kernelEnd(
  const KernelInvocation* kernelInvocation)
{
  // Load default locale
  locale previousLocale = cout.getloc();
  ...
  cout << endl
       ...
       << kernelInvocation->getKernel()->getName() << endl;

  cout << endl << "## Compute" << endl << endl;

  std::vector<std::pair<std::string, size_t>> sorted_ops(m_computeOps.size());
  std::partial_sort_copy(m_computeOps.begin(), m_computeOps.end(),
  ...
  for (auto const& item : sorted_ops)
    cout << "|" << setw(20) << left << item.first << "|" << setw(12) << right
         << item.second << "|" << endl;
  cout << endl;
  ...
  cout << "num unique opcodes required to cover 90\% of dynamic instructions: "
       << major_operations << endl
       << endl;
  ...
  double freedom_to_reorder =
    std::accumulate(m_instructionsBetweenLoadOrStore.begin(),
                    m_instructionsBetweenLoadOrStore.end(), 0.0);
  freedom_to_reorder =
    freedom_to_reorder / m_instructionsBetweenLoadOrStore.size();
  cout << "Freedom to Reorder: " << setprecision(2) << freedom_to_reorder
       << endl
       << endl;
  ...
  // cout << "num barriers hit per thread: " <<
  // m_instructionsToBarrier.size()/m_threads_invoked << endl;
  uint32_t itb_min = *std::min_element(m_instructionsToBarrier.begin(),
                                       m_instructionsToBarrier.end());
  uint32_t itb_max = *std::max_element(m_instructionsToBarrier.begin(),
                                       m_instructionsToBarrier.end());
  ...
  cout << "Instructions to Barrier (min/max/median): " << itb_min << "/"
       << itb_max << "/" << itb_median << endl
       << endl;
  double barriers_per_instruction =
    static_cast<double>(m_barriers_hit + m_threads_invoked) /
    static_cast<double>(total_instruction_count);
  cout << "Barriers per Instruction: " << barriers_per_instruction << endl
       << endl;

  cout << "### Work Distribution" << endl << endl;

  uint32_t ipt_min = *std::min_element(m_instructionsPerWorkitem.begin(),
                                       m_instructionsPerWorkitem.end());
  uint32_t ipt_max = *std::max_element(m_instructionsPerWorkitem.begin(),
                                       m_instructionsPerWorkitem.end());
  ...
  cout << "Instructions per Thread (min/max/median): " << ipt_min << "/"
       << ipt_max << "/" << ipt_median << endl
       << endl;
  ...
  uint16_t simd_min =
    std::min_element(
      m_instructionWidth.begin(), m_instructionWidth.end(),
  ...
  std::transform(m_instructionWidth.begin(), m_instructionWidth.end(),
                 diff.begin(), [simd_mean](const pair_type& x) {
                   return (x.first - simd_mean) * (x.first - simd_mean) *
                          x.second;
                 });
  double simd_sq_sum = std::accumulate(diff.begin(), diff.end(), 0.0);
  double simd_stdev = std::sqrt(simd_sq_sum / (double)simd_num);
  cout << "SIMD Width (min/max/mean/stdev): " << simd_min << "/" << simd_max
       << "/" << simd_mean << "/" << simd_stdev << endl
       << endl;

  double instructions_per_operand =
    static_cast<double>(total_instruction_count) / simd_sum;
  ...
  std::vector<std::pair<size_t, uint32_t>> sorted_count(
    local_address_count[0].size());
  std::partial_sort_copy(local_address_count[0].begin(),
                         local_address_count[0].end(), sorted_count.begin(),
                         sorted_count.end(),
                         [](const std::pair<size_t, uint32_t>& left,
                            const std::pair<size_t, uint32_t>& right) {
                           return (left.second > right.second);
                         });
  ...
  for (auto const& e : sorted_count)
  {
    ...
    // cout << "address: "<< e.first << " accessed: " << e.second << " times!"
    // << endl;
  }

  cout << "num memory accesses: " << memory_access_count << endl << endl;
  cout << "Total Memory Footprint -- num unique memory addresses accessed: "
       << local_address_count[0].size() << endl;
  cout << "                          num unique memory addresses read:     "
       << m_storeOps.size() << endl;
  cout << "                          num unique memory addresses written:  "
       << m_loadOps.size() << endl;
  cout << "                          unique read/write ratio:              "
       << setprecision(4)
       << (float)(((double)m_loadOps.size()) / ((double)m_storeOps.size()))
       << endl;
  cout << "                          total reads:                          "
       << load_count << endl;
  cout << "                          total writes:                         "
  ...
  cout << "                          re-writes:                            "
       << setprecision(4)
       << (float)((double)store_count / (double)m_storeOps.size()) << endl
       << endl;
  size_t significant_memory_access_count =
    (size_t)ceil(memory_access_count * 0.9);
  ...
  cout << "90% Memory Footprint -- num unique memory addresses that cover 90\% "
          "of memory accesses: "
       << unique_memory_addresses << endl
       << endl;
  // cout << "the top 10:" << endl;
  // for (int i = 0; i < 10; i++){
  //    cout << "address: " << sorted_count[i].first << " contributed: " <<
  //    sorted_count[i].second << " accesses!" << endl;
  //}
  ...
  cout << "Global Memory Address Entropy -- measure of the randomness of "
          "memory addresses: "
       << (float)mem_entropy << endl
       << endl;

  cout << "Local Memory Address Entropy -- measure of the spatial locality of "
          "memory addresses"
       << endl
       << endl;
  ...
  for (int nskip = 1; nskip < 11; nskip++)
  {
    ...
    cout << "|" << setw(12) << right << nskip << "|" << fixed << setw(8)
         << setprecision(4) << right << (float)local_entropy << "|" << endl;
  }

  cout << endl << "### Parallel Spatial Locality" << endl << endl;
  ...
  for (size_t i = 0; i < m_psl_per_group[0].size(); i++)
  {
    ...
    avg_psl_i = (avg_psl_i / double(m_psl_per_group.size())) /
                std::log2(double(items_per_group + 1));
    avg_psl.push_back(avg_psl_i);
    avg_psl_sum += avg_psl_i;
    cout << "|" << setw(12) << right << i << "|" << fixed << setw(25)
         << setprecision(4) << right << (float)avg_psl_i << "|" << endl;
  }

  cout << endl << "Normed Locality Sum: " << avg_psl_sum << endl;

  cout << endl
       << "### Memory Diversity -- Usage of local and constant memory relative "
          "to global memory"
       << endl
       << endl;
  ...
  uint32_t m_total_memory_access =
    m_global_memory_access + m_local_memory_access + m_constant_memory_access;

  cout << "\% local memory accesses (local/total): " << setprecision(2)
       << (((float)m_local_memory_access / (float)m_total_memory_access) * 100)
       << endl
       << endl;
  cout << "\% constant memory accesses (constant/total): " << setprecision(2)
       << (((float)m_constant_memory_access / (float)m_total_memory_access) *
           100)
       << endl
       << endl;
  ...
  cout << "Unique Branch Instructions -- Total number of unique branch "
          "instructions to cover 90\% of the branches"
       << endl
       << endl;

  std::vector<std::pair<size_t, uint32_t>> sorted_branch_ops(
    m_branchCounts.size());
  std::partial_sort_copy(m_branchCounts.begin(), m_branchCounts.end(),
                         sorted_branch_ops.begin(), sorted_branch_ops.end(),
                         [](const std::pair<size_t, uint32_t>& left,
                            const std::pair<size_t, uint32_t>& right) {
                           return (left.second > right.second);
                         });
  ...
  for (auto const& x : sorted_branch_ops)
  {
    ...
    cout << "|" << setw(14) << left << x.first << "|" << setw(23) << right
         << x.second << "|" << endl;
  }
  cout << endl;
  ...
  cout
    ...
    << unique_branch_addresses << endl;

  cout << endl
       << "### Branch Entropy -- measure of the randomness of branch "
          "behaviour, representing branch predictability"
       << endl
       << endl;
  ...
  for (auto const& branch : m_branchPatterns)
  {
    for (auto const& h : branch.second)
    {
      ...
      {
        yokota_entropy -= number_of_occurrences * probability_of_taken *
                          std::log2(probability_of_taken);
        yokota_entropy_per_workload -=
          probability_of_taken * std::log2(probability_of_taken);
      }
      // compute linear branch entropy
      float linear_branch_entropy =
        2 * std::min(probability_of_taken, 1 - probability_of_taken);
  ...
  cout << "Yokota Branch Entropy per Workload: " << yokota_entropy_per_workload
       << endl
       << endl;
  ...
  const char* result_path =
    getenv("OCLGRIND_WORKLOAD_CHARACTERISATION_OUTPUT_PATH");
  if (result_path != NULL)
  {
  ...
  {
    int logfile_count = 0;
    logfile_name = "aiwc_" + kernelInvocation->getKernel()->getName() + "_" +
                   std::to_string(logfile_count) + ".csv";
    while (std::ifstream(logfile_name))
    {
      logfile_count++;
      logfile_name = "aiwc_" + kernelInvocation->getKernel()->getName() + "_" +
  ...
  logfile << "Work-groups,Parallelism,"
          << m_group_num[0] * m_group_num[1] * m_group_num[2] << "\n";
  logfile << "Work-items per Work-group,Parallelism,"
          << m_local_num[0] * m_local_num[1] * m_local_num[2] << "\n";
  ...
  logfile << "Max SIMD Width,Parallelism,"
          << std::max_element(m_instructionWidth.begin(),
                              m_instructionWidth.end(),
  ...
  logfile << "Unique Read/Write Ratio,Memory," << setprecision(4)
          << (float)(((double)m_loadOps.size()) / ((double)m_storeOps.size()))
          << "\n";
  logfile << "Total Reads,Memory," << load_count << "\n";
  logfile << "Total Writes,Memory," << store_count << "\n";
  logfile << "Rereads,Memory," << setprecision(4)
          << (float)((double)load_count / (double)m_loadOps.size()) << "\n";
  logfile << "Rewrites,Memory," << setprecision(4)
          << (float)((double)store_count / (double)m_storeOps.size()) << "\n";

  logfile << "90\% Memory Footprint,Memory," << unique_memory_addresses << "\n";
  logfile << "Global Memory Address Entropy,Memory," << mem_entropy << "\n";
  for (int nskip = 1; nskip < 11; nskip++)
  {
    logfile << "LMAE -- Skipped " << nskip << " LSBs,Memory,"
            << loc_entropy[nskip - 1] << "\n";
  }
  for (int nskip = 0; nskip < 11; nskip++)
  {
    logfile << "Normed PSL -- Skipped " << nskip << " LSBs,Memory,"
            << avg_psl[nskip] << "\n";
  }
  ...
  logfile << "Total Local Memory Accessed,Memory," << m_local_memory_access
          << "\n";
  logfile << "Total Constant Memory Accessed,Memory,"
          << m_constant_memory_access << "\n";
  logfile << "Relative Local Memory Usage,Memory,"
          << (((float)m_local_memory_access / (float)m_total_memory_access) *
              100)
          << "\n";
  logfile << "Relative Constant Memory Usage,Memory,"
          << (((float)m_constant_memory_access / (float)m_total_memory_access) *
              100)
          << "\n";
  logfile << "Total Unique Branch Instructions,Control,"
          << sorted_branch_ops.size() << "\n";
  ...
  cout << endl
       << "The Architecture-Independent Workload Characterisation was written "
          "to file: "
       << logfile_name << endl;
  // Restore locale
  cout.imbue(previousLocale);
...
void WorkloadCharacterisation::workGroupComplete(const WorkGroup* workGroup)
{
  ...
  // add the instructions executed per workitem scores back to the global
  // setting
  ...
  if (maxLength != 0)
  {
    for (size_t nskip = 0; nskip < 11; nskip++)
    {
      weighted_avg_psl[nskip] =
        weighted_avg_psl[nskip] / static_cast<float>(maxLength + 1);
    }
  }
