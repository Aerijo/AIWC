...
public:
  ...
  virtual void threadMemoryLedger(size_t address, uint32_t timestep,
                                  Size3 localID);
  virtual void hostMemoryLoad(const Memory* memory, size_t address,
  ...
  virtual void memoryAtomicLoad(const Memory* memory, const WorkItem* workItem,
                                AtomicOp op, size_t address,
                                size_t size) override;
  virtual void memoryAtomicStore(const Memory* memory, const WorkItem* workItem,
                                 AtomicOp op, size_t address,
                                 size_t size) override;
...
private:
  ...
  std::unordered_map<size_t, std::unordered_map<uint16_t, uint32_t>>
    m_branchPatterns;
